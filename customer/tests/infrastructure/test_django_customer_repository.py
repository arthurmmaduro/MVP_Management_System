...
class TestDjangoCustomerRepository(TestCase):
    ...
    def test_soft_delete_marks_customer_as_inactive(self):
        ...
        self.repository.soft_delete(customer.id)
    ...
    def test_soft_delete_raises_customer_delete_failed_on_database_error(self):
        with patch(
            'customer.infrastructure.django_customer_repository.Customer.all_objects.filter'
        ) as filter_mock:
            filter_mock.return_value.update.side_effect = DatabaseError('boom')

            with self.assertRaises(CustomerDeleteFailed) as context:
                self.repository.soft_delete(customer_id=123)

        self.assertEqual(str(context.exception), 'Erro ao deletar o cliente: 123')
